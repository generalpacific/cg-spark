...
public enum DistributionType {
    UNIFORM("uni"), GAUSSIAN("gaus"), CORRELATED("cor"), ANTI_CORRELATED
          ("anti"), CIRCLE("circle");
    ...
    public static DistributionType fromName(final String name) {
        if (StringUtils.isBlank(name)) {
            throw new InvalidParameterException("Name cannot be blank for " +
                    "DistributionType");
        }
        final DistributionType type = map.get(name);
        if (type == null) {
            throw new InvalidParameterException("Invalid name for " +
