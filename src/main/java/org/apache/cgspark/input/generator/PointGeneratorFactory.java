...
public final class PointGeneratorFactory {
  ...
  public PointGeneratorFactory(final Rectangle mbr, final Random random, final double rho) {
    this.mbr = mbr;
    this.random = random;
    this.rho = rho;
    ...
    builder.put(DistributionType.ANTI_CORRELATED,
        new AntiCorrelatedDistPointGenerator(this.mbr, this.random, this.rho));
    builder.put(DistributionType.CIRCLE, new CircleDistPointGenerator(this.mbr,
        this.random));
    builder.put(DistributionType.CORRELATED, new CorrelatedDistPointGenerator(
        this.mbr, this.random, this.rho));
  ...
  public PointGenerator getPointGenerator(final DistributionType type) {
    ...
    if (pointGenerator == null) {
      throw new InvalidParameterException("No PointGenerator present for: "
          + type);
    }
